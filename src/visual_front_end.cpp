    ...
    */
    ...
    VisualFrontEnd::VisualFrontEnd(std::shared_ptr<SlamParams> pstate, std::shared_ptr<Frame> pframe, 
        ...
        : pslamstate_(pstate), pcurframe_(pframe), pmap_(pmap), ptracker_(ptracker), _times(0.0), _averageFeaturePointNums(0.0)
    {}
    ...
    bool VisualFrontEnd::trackMono(cv::Mat &im, double time)
    {
        ...
        std::cout<<"\tprocess current image ID: "<<pcurframe_->id_<<std::endl;
        // int stopID = 158;
        // if(pcurframe_->id_ == stopID){
        //     std::cout<<"stop for checking essential matrix shape"<<std::endl;
        //     exit(-1);
        // }
        // Track the new image
    ...
    void VisualFrontEnd::kltTracking()
    {
        ...
        if( pslamstate_->klt_use_prior_ && !v3dpriors.empty() ) 
        {
            ...
            // _averageFeaturePointNums += v3dpriors.size();
            ...
            _averageFeaturePointNums += nbgood;
        ...
        if( !vkps.empty() ) 
        {
            // _averageFeaturePointNums += vkps.size();
            // Good / bad kps vector
            std::vector<bool> vkpstatus;
    ...
    void VisualFrontEnd::kltTrackingFromKF()
    {
        ...
        if( pslamstate_->klt_use_prior_ && !v3dpriors.empty() ) 
        {
            ...
            size_t nbgood = 0;
            size_t nbkps = v3dkps.size();
            // _averageFeaturePointNums += nbkps;
            for(size_t i = 0 ; i < nbkps  ; i++ ) 
            {
                if( vkpstatus.at(i) ) {
                    pcurframe_->updateKeypoint(v3dkpids.at(i), v3dpriors.at(i));
